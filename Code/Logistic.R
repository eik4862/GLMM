...
log.cv <- function(formula, data.mat, link, fold = 10, iter = 100, prob.cut = seq(0, 1, by = 0.01), x.out = seq(0, 1, by = 0.01), seed = T, parallel = T, verbose = T) {
  # Set seed if needed
  if (seed) set.seed(0)
  ...
  if (parallel) { # Do parallel
    ...
    iter.res <- foreach(i = seq_len(iter), .combine = list, .multicombine = T, 
      ...
      for (j in seq_len(fold)) {  # For each fold
        ...
        # Train
        fit <- glm(formula, data = data.mat[-hold.idx,], family = binomial(link = link))
  ...
  # Report
  if (verbose) {
    cat("\n\tCross Validation Procedure for Logistic Regression\n\n")
...
log.tune <- function(data.mat, link.cand, fold = 10, iter = 5, seed = T, parallel = T, verbose = T) {
  # Set seed if needed
  if (seed) set.seed(0)
  ...
  start <- Sys.time()
  cand <- data.frame(link = link.cand)
  cand$link <- as.character(cand$link)
  ...
  if (parallel) { # Do parallel
    ...
    # Tune
    if (verbose) message("Performing CV")
    iter.res <- foreach (i = seq_len(nrow(cand)), .combine = list, .multicombine = T, .packages = "MASS",
                         .export = c("acc", "tpr.hit", "tpr.miss", "fscore", "mcc", "auc", "logistic.cv"),
                         .options.snow = list(progress = prog)) %dopar% {  # For all combination
      if (seed) set.seed(seed.list[i])
     
      formula <- stepAIC(glm(res ~ ., data.mat, family = binomial(link = cand$link[i])), trace = F)$formula
      cv.res <- logistic.cv(formula, data.mat, cand$link[i], fold = fold, iter = iter, seed = F, parallel = F, verbose = F)$stats[1,]
     
      return(list(formula = formula, cv = cv.res))
    }
    ...
    # join
    formula <- lapply(iter.res, function(x) x$formula)
    names(formula) <- cand$link
    cand <- cbind(cand, t(sapply(iter.res, function(x) unlist(x$cv))))
  } else {  # Do sequential
    formula <- list()
    ...
    for (i in seq_len(nrow(cand))) {  # For all combination
      # Tune
      if (verbose) pb$tick()
      formula[[i]] <- stepAIC(glm(res ~ ., data.mat, family = binomial(link = cand$link[i])), trace = F)$formula
      cand[i,2:7] <- logistic.cv(formula[[i]], data.mat, cand$link[i], fold = fold, iter = iter, seed = F, parallel = F, verbose = F)$stats[1,]
    }
  ...
  sel <- t(data.frame(link = cand$link[max.idx], max = max.val))
  colnames(sel) <- c("ACC", "TPR(Hit)", "TPR(Miss)", "F score", "MCC", "AUC")
  rownames(sel) <- c("link", "max")
  elapsed <- difftime(Sys.time(), start, units = "secs")[[1]]
  
  # Report
  if (verbose) {
    cat("\n\tHyperparameter Tuning Procedure for Logistic Regression\n\n")
    cat(sprintf("  @data    : %s\n", deparse(substitute(data.mat))))
    cat(sprintf("  @fold    : %d\n", fold))
    cat(sprintf("  @iter    : %d\n", iter))
    cat(sprintf("  @parallel: %s\n", as.character(parallel)))
  ...
  return(invisible(list(select = sel, raw = cand, formula = formula)))
}
