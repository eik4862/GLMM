...
ann.data.mat <- function(src) {
  ...
  # Attach mouse encoding
  data <- cbind(data, model.matrix(~data$subj)[,-1])
  colnames(data)[25:28] <- paste0("subj", c(4, 5, 6, 8))
...
ann.cv <- function(data.mat, node, decay, fold = 10, iter = 100, prob.cut = seq(0, 1, by = 0.01), x.out = seq(0, 1, by = 0.01), 
  ...
  if (parallel) { # Do parallel
    ...
    iter.res <- foreach(i = seq_len(iter), .combine = list, .multicombine = T, 
                        .packages = "nnet", .export = c("acc", "tpr.hit", "tpr.miss", "fscore", "mcc", "auc"),
      ...
      for (j in seq_len(fold)) {  # For each fold
        ...
        # Train
        fit <- nnet(res ~ ., data = data.mat[-hold.idx,], decay = decay, size = node, trace = F)
        
        # Test
        pred <- predict(fit, data.mat[hold.idx,])
  ...
  # Report
  if (verbose) {
    cat("\n\tCross Validation Procedure for ANN\n\n")
    ...
    plot(roc$FPR, roc$TPR, main = "Estimated ROC", xlab = "FPR", ylab = "TPR",
         sub = sprintf("AUC: %4f", stats[1,6]), type = "l", col = 2)
...
ann.tune <- function(data.mat, node.cand, decay.cand, fold = 10, iter = 5, seed = T, parallel = T, verbose = T) {
  # Set seed if needed
  if (seed) set.seed(0)
  ...
  start <- Sys.time()
  cand <- data.frame(node = rep(node.cand, times = length(decay.cand)), decay = rep(decay.cand, each = length(node.cand)))
  ...
  if (parallel) { # Do parallel
    ...
    # Tune
    if (verbose) message("Performing CV")
    iter.res <- foreach (i = seq_len(nrow(cand)), .combine = rbind, .packages = "nnet",
                         .export = c("acc", "tpr.hit", "tpr.miss", "fscore", "mcc", "auc", "ann.cv"),
                         .options.snow = list(progress = prog)) %dopar% {  # For all combination
      if (seed) set.seed(seed.list[i])
                           
      ann.cv(data.mat, cand$node[i], cand$decay[i], fold = fold, iter = iter, seed = F, parallel = F, verbose = F)$stats[1,]
    }
    ...
    for (i in seq_len(nrow(cand))) {  # For all combination
      # Tune
      if (verbose) pb$tick()
      cand[i,3:8] <- ann.cv(data.mat, cand$node[i], cand$decay[i], fold = fold, iter = iter, seed = F, parallel = F, verbose = F)$stats[1,]
    }
  }
  ...
  sel <- t(data.frame(node = cand$node[max.idx], decay = log(cand$decay[max.idx]), max = max.val))
  colnames(sel) <- c("ACC", "TPR(Hit)", "TPR(Miss)", "F score", "MCC", "AUC")
  rownames(sel) <- c("node", "decay*", "max")
  elapsed <- difftime(Sys.time(), start, units = "secs")[[1]]
  
  # Report
  if (verbose) {
    cat("\n\tHyperparameter Tuning Procedure for ANN\n\n")
    cat(sprintf("  @data    : %s\n", deparse(substitute(data.mat))))
    cat(sprintf("  @fold    : %d\n", fold))
    cat(sprintf("  @iter    : %d\n", iter))
    cat(sprintf("  @parallel: %s\n", as.character(parallel)))
