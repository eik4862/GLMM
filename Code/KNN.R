...
knn.data.mat <- function(src, pca) {
  ...
  data <- split(data, data$subj, drop = T)
  ...
  # Project data points to PCs
  data <- lapply(seq_along(data), function(i) data.frame(res = data[[i]]$res, as.matrix(data[[i]][,-1]) %*% pca$fit[[i]]$rotation))
  
  return(data)
}

knn.cv <- function(data.mat, pc, k, fold = 10, iter = 100, prob.cut = seq(0, 1, by = 0.01), x.out = seq(0, 1, by = 0.01), seed = T, parallel = T, verbose = T) {
  # Set seed if needed
  if (seed) set.seed(0)
  ...
  if (parallel) { # Do parallel
    ...
    iter.res <- foreach(i = seq_len(iter), .combine = list, .multicombine = T, 
                        .packages = "class", .export = c("acc", "tpr.hit", "tpr.miss", "fscore", "mcc", "auc"),
      ...
      for (j in seq_len(fold)) {  # For each fold
        ...
        # Train
        pred <- lapply(seq_along(data.mat), function(l) {
          knn(data.mat[[l]][-hold.idx[[l]],2:(pc + 1), drop = F], data.mat[[l]][hold.idx[[l]],2:(pc + 1), drop = F], data.mat[[l]]$res[-hold.idx[[l]]], k = k, prob = T)
        })
        
        # Test
        confu.mat <- lapply(seq_along(data.mat), function(k) {
          table(pred = factor(pred[[k]], level = c("Hit", "Miss")), 
        ...
        for (k in seq_along(prob.cut)) {
          confu.mat <- lapply(seq_along(data.mat), function(l) {
            table(pred = factor(ifelse(attr(pred[[l]], "prob") > prob.cut[k], "Hit", "Miss"), levels = c("Hit", "Miss")),
  ...
  # Report
  if (verbose) {
    cat("\n\tCross Validation Procedure for KNN\n\n")
...
knn.tune <- function(data.mat, pc.cand, k.cand, fold = 10, iter = 5, seed = T, parallel = T, verbose = T) {
  # Set seed if needed
  if (seed) set.seed(0)
  ...
  start <- Sys.time()
  cand <- data.frame(pc = rep(pc.cand, times = length(k.cand)), k = rep(k.cand, each = length(pc.cand)))
  ...
  if (parallel) { # Do parallel
    ...
    # Tune
    if (verbose) message("Performing CV")
    iter.res <- foreach (i = seq_len(nrow(cand)), .combine = rbind, .packages = "class",
                         .export = c("acc", "tpr.hit", "tpr.miss", "fscore", "mcc", "auc", "knn.cv"),
                         .options.snow = list(progress = prog)) %dopar% {  # For all combination
      if (seed) set.seed(seed.list[i])
                           
      return(knn.cv(data.mat, cand$pc[i], cand$k[i], fold = fold, iter = iter, seed = F, parallel = F, verbose = F)$stats[1,])
    }
    ...
    for (i in seq_len(nrow(cand))) {  # For all combination
      # Tune
      if (verbose) pb$tick()
      cand[i,3:8] <- knn.cv(data.mat, cand$pc[i], cand$k[i], fold = fold, iter = iter, seed = F, parallel = F, verbose = F)$stats[1,]
    }
  }
  ...
  max.idx <- apply(cand[,3:8], MARGIN = 2, which.max)
  ...
  sel <- t(data.frame(pc = cand$pc[max.idx], k = cand$k[max.idx], max = max.val))
  colnames(sel) <- c("ACC", "TPR(Hit)", "TPR(Miss)", "F score", "MCC", "AUC")
  rownames(sel) <- c("PC", "k", "max")
  elapsed <- difftime(Sys.time(), start, units = "secs")[[1]]
  
  # Report
  if (verbose) {
    cat("\n\tHyperparameter Tuning Procedure for KNN\n\n")
    cat(sprintf("  @data    : %s\n", deparse(substitute(data.mat))))
    cat(sprintf("  @fold    : %d\n", fold))
    cat(sprintf("  @iter    : %d\n", iter))
    cat(sprintf("  @parallel: %s\n", as.character(parallel)))
