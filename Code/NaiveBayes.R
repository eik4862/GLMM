...
naive.cv <- function(data.mat, fold = 10, iter = 100, prob.cut = seq(0, 1, by = 0.01), x.out = seq(0, 1, by = 0.01), seed = T, parallel = T, verbose = T) {
  # Set seed if needed
  if (seed) set.seed(0)
  ...
  if (parallel) { # Do parallel
    ...
    iter.res <- foreach(i = seq_len(iter), .combine = list, .multicombine = T, 
                        .packages = "e1071", .export = c("acc", "tpr.hit", "tpr.miss", "fscore", "mcc", "auc"),
      ...
      for (j in seq_len(fold)) {  # For each fold
        ...
        pred <- lapply(seq_along(data.mat), function(k) {
          predict(naiveBayes(res ~ ., data.mat[[k]][-hold.idx[[k]],]), data.mat[[k]][hold.idx[[k]],], type = "raw")
        })
        
        # Test
        confu.mat <- lapply(seq_along(data.mat), function(k) {
          table(pred = factor(ifelse(pred[[k]][,1] > 0.5, "Hit", "Miss"), level = c("Hit", "Miss")), 
        ...
        for (k in seq_along(prob.cut)) {
          confu.mat <- lapply(seq_along(data.mat), function(l) {
            table(pred = factor(ifelse(pred[[l]][,1] > prob.cut[k], "Hit", "Miss"), levels = c("Hit", "Miss")),
